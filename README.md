...
2. Скачать модель по ссылке и поместить ее в папку: (Или обучить самостоятельно обучить модель по предыдущему шагу)
...
4. Далее нужно запустить контейнер из image, выполнив в консоле следующий код: docker run -it -p 80:80 taa-ml
